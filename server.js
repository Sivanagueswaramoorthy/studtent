...
// NEW: Add Student Route
app.post('/api/admin/add-student', async (req, res) => {
    try {
        await verifyAdmin(req.body.adminToken);
        const { email, full_name, roll_no, department } = req.body;
        
        // Inserts new student. If email already exists, it updates their basic info.
    ...
    } catch (e) { res.status(500).json({ success: false, message: "Failed to add student." }); }
});
